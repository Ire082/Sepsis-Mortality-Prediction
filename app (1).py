...
if st.button("Cargar Datos de Ejemplo"):
    try:
        ...
        # Seleccionar solo el 20% de los pacientes aleatoriamente
        df_clean = df_clean.sample(frac=0.2, random_state=42)
        st.session_state['df'] = df_clean
        st.success("¡Datos cargados exitosamente! ✅")
        st.write(f"Se cargaron {len(df_clean)} pacientes (20% del total)")
...
if st.button("Predecir"):
    if 'df' in st.session_state and selected_predictors:
        try:
            ...
            # Mostrar solo el 20% de los pacientes en la tabla de resultados
            display_df = st.session_state['df'][['subject_id', 'icustay_id', 'Prediction', 'Probability']].sample(frac=0.2, random_state=42)
            st.write(f"Mostrando {len(display_df)} pacientes (20% del total cargado)")
            ...
            if not high_risk.empty:
                # Mostrar solo el 20% de los pacientes de alto riesgo
                high_risk_display = high_risk[['subject_id', 'icustay_id', 'Probability']].sample(frac=0.2, random_state=42)
                st.warning(f"¡Alerta: {len(high_risk)} pacientes con >70% de riesgo de mortalidad! ⚠️")
                st.write(f"Mostrando {len(high_risk_display)} pacientes de alto riesgo (20% del total)")
